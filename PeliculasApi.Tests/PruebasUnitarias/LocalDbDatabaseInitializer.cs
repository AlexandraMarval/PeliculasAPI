...
namespace PeliculasApi.Tests.PruebasUnitarias
{
    public class LocalDbDatabaseInitializer
    {
        ...
        public static PeliculaDbContext GetDbContextLocalDb(bool beginTransaction = true)
        {
            var options = new DbContextOptionsBuilder<PeliculaDbContext>()
                .UseSqlServer($"Data Source=(localdb)\\MSSQLLocalDB;Initial Catalog={_dbName};Integrated Security=True;",
                x => x.UseNetTopologySuite())
                .Options;
            var context = new PeliculaDbContext(options);
            if (beginTransaction)
            {
